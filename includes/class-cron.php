...
class CSS_Cron {
	...
	public function start_scan( bool $manual = false ): void {
		...
		if ( ! empty( $blacklist ) ) {
			$coins = array_values( array_filter( $coins, fn( $c ) => ! in_array( $c['id'], $blacklist, true ) ) );
		}
		...
		if ( empty( $coins ) ) {
			$this->append_scan_log( array(
				...
				'note'        => ! empty( $blacklist )
					? 'همه ارزهای این بازه رتبه در لیست سیاه بودند یا دریافت لیست از CoinGecko ناموفق بود'
					: 'دریافت لیست ارزها از CoinGecko ناموفق بود (به گزارش خطاها نگاه کنید)',
			) );
			return;
		}
	...
	public function process_queue_batch( bool $manual = false ): void {
		...
		foreach ( $batch as $coin ) {
			...
			foreach ( $timeframes as $timeframe ) {
				...
				$closes       = $history_data['close'] ?? array();
				if ( empty( $closes ) ) {
					$coin_failed = true;
					break;
				}
				...
				if ( $this->save_result( $coin, end( $closes ), $analysis, $timeframe, $volume_trend_pct ) ) {
					$saved_count++;
				}
