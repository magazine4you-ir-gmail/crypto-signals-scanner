 ...
 *  1) هر ۱ ساعت هوک css_start_scan اجرا می‌شود -> لیست ۱۰۰ ارز برتر گرفته و در صف قرار می‌گیرد.
 *  2) هر ۱ دقیقه هوک css_queue_worker چند ارز از صف را پردازش می‌کند. برای هر ارز، به ازای
 *     هر تایم‌فریم فعال (ساعتی/روزانه/هفتگی) یک بار جداگانه دیتا گرفته و ذخیره می‌شود.
...
class CSS_Cron {
	...
	/** آیا اسکنی هنوز در حال اجراست (صف خالی نشده و به‌تازگی شروع شده)؟ */
	public function scan_already_in_progress(): bool {
		...
		// اگه بیش از ۳۰ دقیقه از شروعش گذشته، احتمالاً یک اسکن رهاشده/ناتمام قدیمی
		// است (مثلاً به خاطر خطا) — دیگه مانع اسکن تازه نشو
	...
	public function start_scan( bool $manual = false ): void {
		...
		// اگر اسکن خودکار خاموش است و این فراخوانی دستی (دکمه «اسکن الان») نیست، کاری نکن
		if ( ! $manual && empty( $settings['auto_scan_enabled'] ) ) {
			return;
		}

		// محافظ اصلی در برابر مصرف اضافه کریدیت: اگر یک اسکن قبلی هنوز صفش خالی
		// نشده و به‌تازگی (کمتر از ۳۰ دقیقه پیش) شروع شده، صف را از اول نساز —
		// وگرنه همان ارزهایی که همین چند دقیقه پیش پردازش شده‌اند دوباره (و با
		// کریدیت دوباره) پردازش می‌شوند. این دقیقاً همان اتفاقی بود که با کلیک
		// مجدد روی «اسکن الان» قبل از تمام‌شدن اسکن قبلی می‌افتاد.
		if ( $this->scan_already_in_progress() ) {
			return;
		}

		// آخرین وضعیت کلی بازار (مارکت کپ کل + دامیننس بیت‌کوین) را هم در همین چرخه بگیر
		...
		// حذف ارزهای لیست سیاه (مثلاً استیبل‌کوین‌ها) قبل از هر کال گران‌قیمت دیگر
		// (این فیلتر فقط روی همین یک پاسخ /coins/markets که برای کل بازه گرفته شده اعمال
		// می‌شود؛ کال‌های بعدی market_chart/ohlc که واقعاً هزینه اصلی هستند اصلاً برای
		// این ارزها زده نمی‌شوند)
	...
	/** افزودن یک رکورد جدید به تاریخچه اسکن (حداکثر ۳۰ رکورد آخر نگه‌داشته می‌شود) */
	...
	/** به‌روزرسانی آخرین رکورد تاریخچه اسکن (رکورد در حال اجرا) */
	...
	public function process_queue_batch( bool $manual = false ): void {
		...
		// اگر اسکن خودکار خاموش است و این فراخوانی دستی نیست، پردازش صف پس‌زمینه را متوقف کن
		if ( ! $manual && empty( $settings['auto_scan_enabled'] ) ) {
			return;
		}
		...
		foreach ( $batch as $coin ) {
			...
			foreach ( $timeframes as $timeframe ) {
				...
				// درصد استاندارد «قدرت روند» بر پایه حجم معاملات: حجم فعلی نسبت به میانگین
				// متحرک ۲۰ دوره‌ای حجم (RVOL) — همون معیار رایج «Relative Volume». بدون
				// هیچ کال اضافه‌ای محاسبه می‌شود چون داده حجم از همین پاسخ market_chart می‌آید.
			...
			if ( $coin_failed ) {
				$requeue[] = $coin; // این ارز در دور بعد دوباره امتحان می‌شود (برای همه تایم‌فریم‌ها)
			}
		}
	...
	private function save_result( array $coin, float $last_price, array $analysis, string $timeframe, ?float $volume_trend_pct = null ): bool {
		...
		// همگام‌سازی مشخصات پست‌تایپ این ارز (صرف‌نظر از این‌که سیگنال تازه‌ای صادر شده یا نه)
		...
		if ( in_array( $analysis['signal'], array( 'buy', 'sell' ), true ) && $analysis['signal'] !== $previous ) {
			...
			// ثبت سیگنال روی «روز خودش» در تقویم پست‌تایپ این ارز
			CSS_Coin_CPT::record_signal( $coin['id'], $coin['symbol'], $coin['name'], $analysis, $timeframe, $last_price );

			// ثبت همین سیگنال در تقویم هر اندیکاتوری که در صدورش سهیم بوده (پست‌تایپ اندیکاتورها)
	...
	private function log_signal_history( array $coin, float $price, array $analysis, string $timeframe ): void {
		...
		// اندیکاتورهایی که هم‌جهت با سیگنال نهایی بودن (یعنی «صادرکننده» این سیگنال محسوب می‌شن)
		$source = array();
		foreach ( $analysis['details'] as $ind_id => $val ) {
			if ( $val === $analysis['signal'] ) {
				$indicator = CSS_Indicator_Registry::get( $ind_id );
	...
	/** تعداد سیگنال‌های در انتظاری که موعد بررسی‌شان رسیده (برای گزارش پیشرفت به دکمه دستی) */
	...
	public function evaluate_pending_signals(): void {
		...
		foreach ( $pending as $row ) {
			...
			// ثبت نتیجه دقت روی همان روزی که سیگنال اصلی صادر شده بود (تقویم پست‌تایپ ارز)
			...
			// همین نتیجه را در تقویم هر اندیکاتور صادرکننده هم ثبت کن
