		...
		<div class="wrap css-wrap">
			...
			<form method="post" action="options.php">
				...
				<table class="form-table">
					...
					<tr>
						<th><label for="rank_start">بازه رتبه ارزها (بر اساس مارکت کپ)</label></th>
						<td>
							از رتبه
					...
					<tr>
						...
						<td>
							...
							<p class="description">
								شناسه CoinGecko هر ارز را در هر خط (یا با کاما جدا) بنویسید — نه نماد، بلکه همان id که در آدرس coingecko.com/coins/... می‌آید (مثلاً برای تتر: <code>tether</code>).
					...
					document.getElementById('css-fill-stablecoins')?.addEventListener('click', function () {
						...
						var common = ['tether', 'usd-coin', 'dai', 'binance-usd', 'true-usd', 'usdd', 'first-digital-usd', 'paypal-usd', 'ethena-usde', 'frax', 'gemini-dollar', 'paxos-standard'];
					...
					<tr>
						...
						<p class="description">در صورت خالی بودن، از API عمومی و رایگان با محدودیت نرخ پایین‌تر استفاده می‌شود.</p></td>
					</tr>
					<tr>
						<th><label for="api_base_url">آدرس پایه API (پراکسی سفارشی)</label></th>
						<td><input type="text" id="api_base_url" style="width:350px" placeholder="https://api.coingecko.com/api/v3" name="css_settings[api_base_url]" value="<?php echo esc_attr( $settings['api_base_url'] ?? '' ); ?>">
						<p class="description">اگر سرور شما به CoinGecko دسترسی مستقیم ندارد (خطای timeout)، آدرس یک پراکسی (مثلاً Cloudflare Worker) را اینجا وارد کنید. خالی بگذارید تا از آدرس پیش‌فرض CoinGecko استفاده شود.</p></td>
					</tr>
					<tr>
						...
						<p class="description">برای جلوگیری از خطای ۴۲۹ (Rate Limit)، حداقل ۲۵۰۰ میلی‌ثانیه پیشنهاد می‌شود. اگر کلید API پولی دارید می‌توانید کمترش کنید.</p></td>
					</tr>
				</table>

				<h2>اندیکاتورهای فعال</h2>
				<p class="description">⚠️ اندیکاتور SuperTrend به داده High/Low نیاز دارد و برای هر ارز یک درخواست اضافی به CoinGecko می‌زند — یعنی با فعال‌بودنش، اسکن کندتر می‌شود و احتمال برخورد به Rate Limit بیشتر می‌شود. اگر با ۴۲۹ مواجه شدید، عدد «فاصله بین درخواست‌ها» را بالاتر ببرید.</p>
