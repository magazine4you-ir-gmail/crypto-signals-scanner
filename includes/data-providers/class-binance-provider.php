 ...
 * حالا هم نماد پایه (BTC) و هم نماد کامل (BTCUSDT) را قبول می‌کند.
 */
class CSS_Binance_Provider implements CSS_Data_Provider_Interface {
    ...
    private function get_quote(): string {
        return strtoupper( trim( $this->settings['binance_quote_asset'] ?? 'USDT' ) );
    }

    /**
     * هر ورودی را به نماد کامل بایننس تبدیل می‌کند
     */
    private function to_binance_symbol( string $asset ): string {
        return CSS_Coin_Identity::to_binance_symbol( $asset, $this->get_quote() );
    }
    ...
    private function fetch_klines( string $symbol, int $days, string $timeframe ): array {
        $symbol   = $this->to_binance_symbol( $symbol );
    ...
    public function get_current_price( string $asset ): ?float {
        $symbol = $this->to_binance_symbol( $asset );
        $response = $this->gateway_get( '/?action=ticker&symbols=' . rawurlencode( $symbol ) );
